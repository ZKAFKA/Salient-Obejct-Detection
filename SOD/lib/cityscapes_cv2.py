...
class CityScapes(BaseDataset):
    '''
    '''
    def __init__(self, dataroot, trans_func=None, mode='train'):
        super(CityScapes, self).__init__(
                dataroot, trans_func, mode)
        self.n_cats = 19
        self.lb_ignore = 255
...
def get_data_loader(datapth, ims_per_gpu, scales, cropsize, mode='train'): #, distributed=True):
    if mode == 'train':
    ...
    dss = CityScapes(datapth, trans_func=trans_func, mode=mode)
    count=0
    dl = DataLoader(dss,batch_size=batchsize,shuffle=True,drop_last=drop_last,num_workers=0, pin_memory=True)
