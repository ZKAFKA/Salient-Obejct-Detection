...
def main():
    global cfg, best_mae, best_epoch, scheduler
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS):
        ...
        heads, mious, miou = eval_model(model, val_loader, epoch)
        print("epoch{} mIOU: {}".format(epoch + 1, miou))
        ...
        save_file = os.path.join(
            check_point_dir, 'BANet_checkpoint_epoch{}.pth.tar'.format(epoch + 1))
        save_checkpoint(
            {
                'epoch': epoch + 1,
                'arch': cfg.MODEL.ARCH,
    ...
    for i, data in enumerate(train_loader):
        ...
        if cfg.LOSS.MLOSS > 1:
            for k in range(cfg.LOSS.MLOSS):
                loss = loss + F.binary_cross_entropy_with_logits(output[k], target)
        else:
            loss = loss + F.binary_cross_entropy_with_logits(output, target)
...
#                         size=(h[idx], w[idx]),
#                         mode='bilinear',
#                         align_corners=False) * 255.0).int().float() / 255.0
